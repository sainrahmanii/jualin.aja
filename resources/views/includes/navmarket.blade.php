<nav class="bg-white shadow-lg fixed top-0 left-0 right-0 z-10">
    <div class="max-w-6xl mx-auto py-3">
        <div class="flex justify-between items-center">
            <!-- logo -->
            <button>
                <a href="/"><img src="{{ asset('assets/images/jualin.aja.png') }}" alt="logo"
                        class="lg:w-64 w-40 pl-6 lg:pl-0"></a>
            </button>
            ...
            <div class="md:flex">
                <label class="relative block">
                    ...
                    <input
                        class="placeholder:text-slate-400 block bg-white lg:w-96 w-56 border border-slate-300 rounded-full py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-primary focus:ring-1 sm:text-sm"
            ...
            <div class="md:flex space-x-4 hidden items-center">
                <div class="md:flex space-x-4 hidden items-center">
                    ...
                    @else
                        <a href="{{ route('sign-in') }}"><button
                                class="rounded-full border border-primary text-primary font-bold py-2 px-9">Masuk</button></a>
                        <a href="{{ route('sign-up') }}"><button
    ...
    <div class="hidden mobile-nav justify-center pb-5">
        @auth
            ...
            <a href="#" class="flex justify-center mx-auto bg-primary rounded-full py-2 w-1/2 mt-3"
                onclick="event.preventDefault(); document.getElementById('logout-form').submit()">Keluar
                <form method="POST" action="{{ route('logout') }}" id="logout-form">
                    @csrf
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
        ...
        @else
            <a href="{{ route('sign-in') }}" class="block py-2 pl-32"><button
                    class="border border-primary rounded-full py-2 px-20 font-bold">Masuk</button></a>
            <a href="{{ route('sign-up') }}" class="block py-2 pl-32"><button
                    class="rounded-full bg-primary text-white py-2 px-20 font-bold">Daftar</button></a>
        @endauth
    </div>
